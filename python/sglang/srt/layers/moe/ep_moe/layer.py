...
class NpuFuseEPMoE(DeepEPMoE):
    ...
    def forward(
        ...
        # Handle different weight scale attribute names based on quantization config
        w13_weight_scale = (
        ...
        return self.dispatcher.dispatch(
            ...
            gmm1_permuted_weight_scale=w13_weight_scale,
            gmm2_weight=self.w2_weight,
            gmm2_weight_scale=w2_weight_scale,
        ).hidden_state
    ...
    def _process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # Handle different weight scale attribute names based on quantization config
        # Use w13_weight_scale_inv for block quantization, w13_weight_scale for per-tensor
        w13_scale_attr = (
        ...
        if hasattr(layer, w13_scale_attr):
            w13_scale = getattr(layer, w13_scale_attr).data.squeeze(-1).contiguous()
            w13_scale = self.permute_w13_weight_scale(w13_scale, 128)
            setattr(
                layer,
                w13_scale_attr,
                torch.nn.Parameter(w13_scale.to(torch.float32), requires_grad=False),
            )

        if hasattr(layer, w2_scale_attr):
            w2_scale = getattr(layer, w2_scale_attr).data.squeeze(-1).contiguous()
            setattr(
                layer,
                w2_scale_attr,
                torch.nn.Parameter(w2_scale.to(torch.float32), requires_grad=False),
            )
